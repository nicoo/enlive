...
(defn- jattrs->saxattrs
  ...
  (let [sax-attrs (org.xml.sax.ext.Attributes2Impl.)]
    (doseq [attr jattrs]
      (.addAttribute sax-attrs
                     ""
                     (.getName attr)
                     (.getKey attr)
                     "CDATA"
...
(defn- startparse-jericho
  "parses html using jericho and calls the sax handler as jericho
   does not support it out of the box."
  [s ch]
  (let [stream (.getByteStream s)
        ss (net.htmlparser.jericho.StreamedSource. stream)]
    (doseq [el ss]
      (condp instance? el
        ...
        (let [type (.getTagType el)
          ...
          (when (or
                 ...
                 (= net.htmlparser.jericho.EndTagType/NORMAL type))
            (condp instance? el
              ;; start element
              net.htmlparser.jericho.StartTag
              (let [attrs (jattrs->saxattrs
                           (.getAttributes ^net.htmlparser.jericho.StartTag el))]
                (.startElement ch "" tag-name tag-name attrs))

              ;; end element
              net.htmlparser.jericho.EndTag
              (.endElement ch "" tag-name tag-name))))

        ;; Segment or CharacterEntityReference
        ;; simply append text.
        net.htmlparser.jericho.Segment
        (let [c-arr (.toCharArray (str el))]
          (.characters ch c-arr 0 (count c-arr)))))))

(def ^:dynamic *parse-fn* startparse-tagsoup)

(defmacro with-parse-fn
  [parse-fn & body]
  `(binding [*parse-fn* ~parse-fn]
     ~@body))
 ...
 [stream]
  (filter map?
    (with-open [^java.io.Closeable stream stream]
      (xml/parse (org.xml.sax.InputSource. stream) *parse-fn*))))
